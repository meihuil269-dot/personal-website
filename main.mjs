...
const photoWallDialog = document.querySelector('#photo-wall-dialog')
...
const lifePhotoIds = ['001','002','003','004','005','006','007','008','009','010','011','012','013','014','015','016','017','018','019','020','021','022','023','024','025','026','027','028','029','030','031','032','033','034','035','036','037','038','039','040','041','042','043','044','046','047','048','049','050','051','052','053','054','055','056','057','058','059','060','061','062','063','064','065','066','067','069','071','072','074','075','076','077','078','079','080','081','082','083','084','085','086','087','088','090','091','092','093','094','095','096','097','098','099','100','102','103','104','105','106','107','108','109','111','113','114','115','116','117','118','119','120','121','122']
const lifeGalleryGrid = document.querySelector('#life-gallery-grid')
lifeGalleryGrid.innerHTML = lifePhotoIds.map((id) => `<a href="./assets/life-photos/photo-${id}.jpg" target="_blank" rel="noopener noreferrer"><img src="./assets/life-photos/photo-${id}.jpg" loading="lazy" alt="陈璐的生活照片 ${Number(id)}" /></a>`).join('')
document.querySelector('[data-open-photo-wall]').addEventListener('click', () => { photoWallDialog.hidden = false })
document.querySelectorAll('[data-close-photo-wall]').forEach((button) => button.addEventListener('click', () => { photoWallDialog.hidden = true }))
document.querySelectorAll('.interest-card:not([data-open-bookshelf]):not([data-open-film]):not([data-open-music]):not([data-open-photo-wall])').forEach((card) => card.addEventListener('click', () => openDetail(card.dataset.interest, 'OFF THE CLOCK', card.dataset.detail)))
...
document.querySelectorAll('.filter-row:not(.work-filter) > button').forEach((button) => button.addEventListener('click', () => {
  ...
  document.querySelectorAll('.timeline article').forEach((article) => {
    const matchesMedia = type === 'media' && article.classList.contains('media-entry')
    article.hidden = type !== 'all' && !matchesMedia && !article.dataset.type.split(' ').includes(type)
  })
}))
